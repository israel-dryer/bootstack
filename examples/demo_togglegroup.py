...
class ToggleGroupDemo(bs.App):
    def __init__(self):
        ...
        bs.Label(self, text="Single Mode (View Selector)", font="-weight bold").pack(pady=(10, 5))
        
        view_signal = bs.Signal("grid")
        
        single_group = bs.ToggleGroup(
            self,
            mode='single',
            signal=view_signal
        )
        ...
        single_label = bs.Label(self, text=f"Selected View: {view_signal.get()}")
        single_label.pack()
        view_signal.subscribe(lambda v: single_label.configure(text=f"Selected View: {v}"))

        # --- 2. Multi Mode (CheckToggle) with Signal ---
        bs.Label(self, text="Multi Mode (Filters)", font="-weight bold").pack(pady=(10, 5))

        filter_signal = bs.Signal({'on_sale'})

        multi_group = bs.ToggleGroup(
            self,
            mode='multi',
        ...
        multi_label = bs.Label(self, text=f"Selected Filters: {filter_signal.get()}")
        multi_label.pack()
        ...
        # Outline style
        outline_group = bs.ToggleGroup(style_frame, variant='outline', value='b')
        outline_group.add("A", "a")
        outline_group.add("B", "b")
        outline_group.pack(side='left', padx=10)

        # Ghost style
        ghost_group = bs.ToggleGroup(style_frame, variant='ghost', accent='success', value='y')
        ghost_group.add("X", "x")
        ghost_group.add("Y", "y")
        ghost_group.pack(side='left', padx=10)

        # --- 4. Different Orientations ---
        bs.Label(self, text="Vertical Orientation", font="-weight bold").pack(pady=(10, 5))

        vertical_group = bs.ToggleGroup(
            self,
            mode='multi',
            orient='vertical',
            accent='danger',
