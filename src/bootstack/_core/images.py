...
class _ImageService:
    ...
    _icon_metrics: dict[str, list[float]] | None = None
    ...
    @classmethod
    def _get_icon_metrics(cls) -> dict[str, list[float]]:
        """Lazy-load precomputed normalized ink bounds per glyph.

        Maps icon name to `[left, top, width, height]` as fractions of the font
        size (from the text draw origin), generated by
        `tools/generate_icon_metrics.py`. Missing file degrades gracefully to an
        empty map (the renderer then falls back to measuring at render time).
        """
        if cls._icon_metrics is None:
            try:
                cls._icon_metrics = json.loads((_ICONS_DIR / "icon_metrics.json").read_text(encoding="utf-8"))
            except (OSError, ValueError):
                cls._icon_metrics = {}
        return cls._icon_metrics
    ...
    def _render_icon(
        ...
        metrics = cls._get_icon_metrics().get(name)
        if metrics:
            # Precomputed normalized ink bounds (em fractions, from the text
            # draw origin): [left, top, width, height]. Fit the ink to the inner
            # box (never enlarging past natural size) and center on the true ink
            # — accurate for full-bleed glyphs and free of getbbox skew.
            nl, nt, nw, nh = metrics
            font_size = max(1, int(min(canvas_size, inner / max(nw, nh, 1e-6))))
            font = cls._get_icon_font(font_size)
            ink_w, ink_h = nw * font_size, nh * font_size
            dx = (canvas_size - ink_w) / 2 - nl * font_size
            ...
            # works). Regenerate the metrics with tools/generate_icon_metrics.py.
            ...
            if glyph_w > inner or glyph_h > inner:
                scale_factor = min(inner / max(glyph_w, 1), inner / max(glyph_h, 1)) * 0.95
