...
def build_checkbutton_style(
    b: BootstyleBuilderTTk, ttk_style: str, accent: str = "primary", **options
):
    ...
    on_normal = b.on_color(normal)
    on_hovered = b.on_color(hovered)
    ...
    if show_indicator:
        ...
        # Magenta channel = outer background area (shows as focus halo when focused).
        # Black channel = checkbox foreground/fill color.
        # Transparent pixels = gap + unchecked interior, rendered over widget background.
        normal_checked_img = recolor_element_image("checkbox_checked", on_normal, normal, background)
        normal_unchecked_img = recolor_element_image("checkbox_unchecked", background, foreground, background)
        normal_indeterminate_img = recolor_element_image("checkbox_indeterminate", on_normal, normal, background)

        hovered_checked_img = recolor_element_image("checkbox_checked", on_hovered, hovered, background)
        hovered_unchecked_img = recolor_element_image("checkbox_unchecked", background_hover, foreground, background)
        hovered_indeterminate_img = recolor_element_image("checkbox_indeterminate", on_hovered, hovered, background)

        pressed_checked_img = recolor_element_image("checkbox_checked", on_pressed, pressed, background)
        pressed_unchecked_img = recolor_element_image("checkbox_unchecked", background_hover, foreground, background)
        pressed_indeterminate_img = recolor_element_image("checkbox_indeterminate", on_pressed, pressed, background)

        focus_checked_img = recolor_element_image("checkbox_checked", on_normal, normal, focused)
        focus_unchecked_img = recolor_element_image("checkbox_unchecked", background, foreground, focused)
        focus_indeterminate_img = recolor_element_image("checkbox_indeterminate", on_normal, normal, focused)

        disabled_checked_img = recolor_element_image("checkbox_checked", on_disabled, disabled, background)
        ...
        disabled_indeterminate_img = recolor_element_image(
            "checkbox_indeterminate", on_disabled, disabled, background
        )

        b.create_style_element_image(
            ElementImage(
                f"{ttk_style}.indicator", normal_unchecked_img.image, sticky="ns"
            ).state_specs(
                [
                    # Disabled states
