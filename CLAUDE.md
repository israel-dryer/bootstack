...
### ★ START HERE (2026-08-28) — **#486 IS ON A BRANCH AWAITING ITS FIRST REVIEW ROUND. `0.4.0` HAS TWO ISSUES LEFT: #486 AND #467.**

#### ⭐⭐ #486 — **IMPLEMENTED, PUSHED, NOT REVIEWED, NO PR.** Branch `fix/textarea-signal-binding-486`, head **`51631a07`**. **Cap 3, spent 0 — round 1 is the next thing anyone does on this branch.**

**READ `PLAN.md` AT THE BRANCH ROOT FIRST.** It is the reviewer's inbound artifact and carries the whole design, the measurements and the two rejected alternatives. **Do not re-derive them.** ⚠ **Archive `PLAN.md` and `REVIEW.md` into `development/` BEFORE the PR opens** — #460 merged without that step and left a stale plan in `main`'s root, which is the third time in five branches.
...
⚠ **`fix/textarea-signal-binding-486` measures `1680 / 22`** — `1661 + 19`, its one new test file,
bounded the same way. **That is a BRANCH figure; do not record it as `main`'s.**
