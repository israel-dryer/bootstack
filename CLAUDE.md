...
### Pre-work before v2 starts

1. ~~`Style._tk_widgets` WeakSet + visibility guard~~ — done (PR #53)
2. `autostyle` opt-out should be **container-level** (`autostyle_children=False`)
   rather than per-widget `autostyle=False`. Canvas widgets (Slider, Meter)
   needed it manually; new ones will too. Clean this up before v2.
