...
- **A `Signal` can now hold an empty value, so clearing a field is no longer silent.** A signal takes its type from the value you create it with, and it had no way to say "nothing" — so a field bound to one kept its last value when it was cleared, and a handler watching that signal never heard about the clear. Pass `nullable=True` when the value can also be empty: `bs.Signal(date.today(), nullable=True)`. Clearing a bound field then sets the signal to `None` and notifies its subscribers, and writing `None` to the signal empties the field. A nullable signal can also start empty — `bs.Signal(None, nullable=True)` — in which case the first real value decides its type, and it stays that type from then on. Signals you already have are unchanged: without `nullable=True` a clear is still skipped and `set(None)` still raises, now with a message pointing at the option. This is for the fields that carry a typed value — `NumberField`, `DateField`, `TimeField`, `Select` and `SelectButton`. Binding a nullable signal to a text field or a checkbox raises rather than storing something that is not empty at all: those widgets keep their value in the signal's own variable, which cannot hold an empty one. A text field is already empty at `''`. ([#390](https://github.com/israel-dryer/bootstack/issues/390))
