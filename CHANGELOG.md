...
- **A `Signal` can now hold an empty value, so clearing a field is no longer silent.** A signal takes its type from the value you create it with and had no way to say "nothing", so a field bound to one kept its last value when it was cleared and nothing watching the signal heard about it. Pass `allow_empty=True` when the value can also be empty — `bs.Signal(date.today(), allow_empty=True)` — and `clear()` empties it — a clear now travels in both directions between field and signal. A signal can also start empty, in which case name its type: `bs.Signal(None, allow_empty=True, dtype=date)`. Empty reads as `None`, except where the signal *is* the widget's own variable, as it is for a text field or a radio group, where it reads as `''` — and a signal holding a `set`, as a multi-select toggle group does, empties to the empty set. A falsiness check covers all three. Signals you already have are unchanged: without `allow_empty=True`, `clear()` and `set(None)` raise, and clearing a bound field leaves the signal alone. Binding a signal that allows an empty value to a checkbox, switch, toggle button, slider or progress bar raises — a checkbox is off or on, and a slider always has a position, so there is no empty for them to hold. ([#390](https://github.com/israel-dryer/bootstack/issues/390))
- **`Select` gains public surface it was missing.** From [#465](https://github.com/israel-dryer/bootstack/issues/465): `select.valid` and `select.error`, both bindable `Signal`s, the `on_valid`, `on_invalid` and `on_validate` events, and the field addon API — `insert_addon`, `update_addon`, `remove_addon` and `addons`. From [#458](https://github.com/israel-dryer/bootstack/issues/458): `select.signal`, which reads back the `Signal` a `Select` was built with, as `NumberField`, `DateField` and `TimeField` already allowed. Each one is explained in the `Fixed` entry it came from.

### Changed

- **A keyword a widget does not recognize is now reported instead of ignored.** `bs.TextField(bogus=1)` used to construct as if nothing had happened, and so did a typo of a real parameter — `bs.DataTable(densty="compact")` silently used default spacing, `bs.Label("hi", filll="x")` silently ignored the placement. The name was dropped on the floor, which made the public layer less strict than the widgets underneath it, where the same typo has always raised. Every widget you place in a layout now raises `TypeError` naming itself and the keyword it did not recognize. Nothing accepts an unknown keyword in silence any more, but the top-level windows — `App`, `AppShell`, `Workbench` and `Window` — report it through the toolkit they are built on, so their message names that rather than the window. **Check this when you upgrade: an application that starts today can fail to start after the upgrade** — but only where a keyword was already doing nothing, so what you get back is a message pointing at the typo instead of a setting that silently never applied. Placement keywords (`grow=`, `horizontal=`, `row=`, `column=` and the rest) are unaffected, and `Chart`, `MenuButton`, `Picture`, `StatusBar` and `Toolbar` still pass extra options through to what they wrap. ([#472](https://github.com/israel-dryer/bootstack/issues/472))

- **A `SelectButton` bound to a `Signal` now carries the option's value, not the label shown for it.** The same defect as the `Select` fix in this release, and it shows up wherever an option's label differs from its value: seeding the signal with a value displayed that raw value on the button and left `selection` empty for good, while selecting `Three` put the label `'Three'` into the signal where `value` reported `'3'`. The signal now carries the value in both directions. **Check this when you upgrade if you seed a `SelectButton` signal with anything other than one of its option values** — a label, or any other string, now raises; seed it with the option's value instead. This reaches buttons built from plain strings too, where the label and the value are the same text. A button whose seed already names an option is unaffected, as is any button with no `signal=`. Clearing the button in code leaves an ordinary signal untouched; declare it `allow_empty=True` and the clear reaches it (see the `Signal` entry under `Added`). Reading `button.signal` back now returns the `Signal` you passed in rather than a separate one tracking it. ([#461](https://github.com/israel-dryer/bootstack/issues/461))

### Fixed

- **A validation field no longer stops validating when a `custom` rule cannot judge a value.** A `custom` rule runs a function you supply, and if that function raised — comparing a number against text it never expected, say, or meeting an empty field it did not handle — the exception escaped. On an automatic trigger there is no call of yours for it to escape to: it unwound into the event loop, and the field silently kept whatever validity it already had. The user saw a field that had quietly stopped responding, with nothing to explain it. A function that raises now reports the value invalid, and the first time one of your functions raises, a single line naming the exception is written to stderr so the failure is not silent — set `BOOTSTACK_DEBUG` for the full traceback. The field shows "Could not check this value (expected: must be over 5)", carrying the `message` you gave the rule as an expectation rather than as a verdict: a function that raised never established anything about this value, so reporting your message on its own can tell the user something false — a field reading "must be over 5" while holding 6. Your message is shown on its own, as the verdict, when your function actually returns `False`. **Check this when you upgrade if you attach a `custom` rule to an optional field and your function does not handle an empty value: that field now reports invalid where it previously appeared valid.** The same guard applies when you call `validate()` yourself, so a raising function returns `False` there instead of propagating to your call site. ([#467](https://github.com/israel-dryer/bootstack/issues/467))
- **A `Signal` bound to a `TextArea` or `CodeEditor` now works in both directions, and reading `.signal` back gives you the signal.** Both widgets take a `textsignal=` and the docs describe that as a two-way binding, but only one direction was wired: the widget followed the signal, while typing in the widget left the signal at its old value forever. Their `.signal` property was worse — it returned `None` in every case, including when a signal was bound, so there was no way to ask a text area what it was bound to. Edits now travel back to the signal as you make them, the same as a single-line text field, and `.signal` returns the signal you bound (still `None` when nothing is bound). **Check this when you upgrade if you subscribe to a signal bound to one of these two widgets: that subscriber starts firing on user edits, where before it only heard writes your own code made.** One narrower change comes with it: binding a signal that holds something other than text — `bs.TextArea(textsignal=bs.Signal(123))` — now raises `TypeError` when you build the widget, because the text written back could never be stored in it. Single-line fields have always refused the same thing with the same message. One more thing this fixes: writing to a bound signal while a `TextArea`'s placeholder was on screen left the widget stuck. The text appeared, but the widget went on treating itself as empty — `value` returned an empty string, and `on_input` and `on_change` stopped firing for anything the user typed after that. Both behave normally now. ([#486](https://github.com/israel-dryer/bootstack/issues/486))

- **Reading `.signal` on a field or a boolean control no longer looks like it might hand back nothing.** `TextField`, `PasswordField`, `PathField`, `SpinnerField`, `Checkbox`, `Switch` and `ToggleButton` all documented `.signal` as returning a `Signal` *or* `None`, and none of them can return `None` — they create a signal on first read whether or not you bound one. A type checker made you guard a case that cannot happen, and the API Reference described one. The seven now say what they do. Widgets that really can hand back `None` are unchanged and still say so: `TextArea`, `CodeEditor`, `NumberField`, `DateField`, `TimeField`, `Select` and `SelectButton`. Behavior is identical either way — this corrects what the widgets claim, not what they do. ([#460](https://github.com/israel-dryer/bootstack/issues/460))

- **A modal `bs.Window` now hands the grab back to whatever was modal underneath it.** Opening a modal window from inside another modal — an "Advanced…" button on a dialog, say — left the dialog underneath on screen and still blocking the code that opened it, but no longer modal: the user could click straight past it into the main window and drive the application. The window took the modal grab and never gave it back when it closed, and the toolkit does not restore a grab that was displaced, only drop the one being released. A closing modal window now returns the grab to its previous holder, and returns it as the same kind it was, so an application-modal window underneath stays application-modal rather than quietly narrowing. This was the same defect fixed for dialogs in `0.3.1`, on the one path that fix did not cover; a window with no modal opener, and any non-modal window, are unaffected. ([#444](https://github.com/israel-dryer/bootstack/issues/444))
- **A `SelectButton` no longer reports the same selection twice.** Every change fired `on_change` two times — once for each of two handlers the button had accidentally registered on itself while being built — so a handler that saves a record, sends a request, or increments a counter did all of it twice for a single selection. This applied to every `SelectButton`, whichever way the options were written, and both to a selection made from the menu and to one set in code. A selection is now announced exactly once. Nothing else changes: the event, its payload and its timing are the same as before, there is just one of them. ([#476](https://github.com/israel-dryer/bootstack/issues/476))
- **A `TimeField` bound to a `Signal` no longer reports a change on startup.** Seeding the field from its signal announced a change event while the field was being built, for a time nobody had picked — so an application reacting to a time change ran that reaction once at startup, before the user had touched anything. A handler registered on the line after the constructor still received it, because the event waited in the queue until the application began running. Construction is not a change: `TimeField(signal=…)` is now silent, matching `TimeField(value=…)`, which never announced one, and matching `NumberField` and `DateField`, which were already silent. Every change after construction is announced exactly as before. ([#459](https://github.com/israel-dryer/bootstack/issues/459))
...
- **A validation rule on a `Select` now has somewhere to report.** `Select` accepted `add_validation_rule()` and ran the rule, but exposed no `valid` or `error` to read the outcome from and emitted no `valid`/`invalid` events, so a failing rule had no way to reach your code or the screen. It now carries the same validation surface as the rest of the field family — `select.valid` and `select.error` are `Signal`s you can bind (`bs.Label(textsignal=select.error)`), and `on_valid`, `on_invalid` and `on_validate` fire as they do on `TextField`. `Select` also gains the field addon API (`insert_addon`, `update_addon`, `remove_addon`, `addons`), which it had been using internally all along for its own dropdown arrow. The rules you attach today run exactly as before — this only gives their outcome somewhere to go. One thing to check when you upgrade — `Select` now shares the field family's `add_validation_rule()`, so passing anything other than a rule-type string raises `TypeError` instead of being accepted and then silently ignored. ([#465](https://github.com/israel-dryer/bootstack/issues/465))
- **A `Select` bound to a `Signal` now tracks the selection, not the text on screen.** Seeding the signal with an option's value displayed that raw value instead of the option's label, and setting it later moved the displayed text without moving the selection — so the field showed one option while `value` and `selection` still reported the previous one, no change event fired, and it did not correct itself. That second half applied to plain string options too. The signal now carries the option's value in both directions, matching `value=` and the `signal=` on `NumberField`, `DateField` and `TimeField`. Two things to check when you upgrade — what a `Select` writes into its signal changes from the option's label to its value, and passing `textsignal=` to a `Select` now raises instead of being silently discarded. Clearing the field leaves an ordinary signal untouched; declare it `allow_empty=True` and the clear reaches it. `SelectButton` had the same defect and is fixed with it. ([#458](https://github.com/israel-dryer/bootstack/issues/458))
