...
- **The keypad Enter key now presses a dialog's default button.** Only the main Enter key did. The two are separate keys as far as the toolkit is concerned — on Windows, Linux and macOS alike — and only one of them was wired up, so a dialog filled in from the number pad had to be finished with the mouse or with the other Enter. Both now trigger the default button, through the same command and the same refusal path as a click.
