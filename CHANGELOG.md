...
- **A `Signal` can now hold an empty value, so clearing a field is no longer silent.** A signal takes its type from the value you create it with, and it had no way to say "nothing" — so a field bound to one kept its last value when it was cleared, and a handler watching that signal never heard about the clear. Pass `allow_empty=True` when the value can also be empty: `bs.Signal(date.today(), allow_empty=True)`. Call `clear()` to empty it — the same verb the fields themselves use — and clearing a bound field now reaches the signal, in both directions. A signal that allows an empty value can also start empty, in which case there is no value to take its type from and you name it with `dtype`: `bs.Signal(None, allow_empty=True, dtype=date)`. `dtype` is honored whenever it is given, so a seed that may or may not be there — `bs.Signal(record.get("due"), allow_empty=True, dtype=date)` — needs no second spelling, and a seed that contradicts it is reported at construction rather than at some later write. Empty is `None`, except where the signal *is* the widget's own variable, as it is for a text field or a radio group: a variable holds only strings, so there empty is `''`. A `str` is the only type that can be either, and where one signal is both — a `Select`'s value and a label's text — the variable wins, because a variable has no way to hold `None`; a falsiness check reads the same in every case. Signals you already have are unchanged: without `allow_empty=True` a clear is still skipped and `set(None)` still raises, now with a message pointing at the option. Binding a signal that allows an empty value to a checkbox, switch, toggle button, slider or progress bar raises rather than storing something that is not empty at all — those types have no empty value: a checkbox is off or on, and a slider always has a position. ([#390](https://github.com/israel-dryer/bootstack/issues/390))
...
- **A `SelectButton` bound to a `Signal` now carries the option's value, not the label shown for it.** This is the same defect as the `Select` fix in this release, in the button-styled sibling, and it shows up wherever an option's displayed label differs from its stored value. Seeding the signal with a value — the thing `value=` takes, and what the documentation promised — displayed that raw value on the button and left `selection` empty for good, so the button never considered anything selected. Seeding it with the label worked, and was the only spelling that did. Writing back had the same error in reverse: selecting `Three` put the label `'Three'` into the signal where `value` reported `'3'`, so code reading the signal and code reading `value` disagreed about the same selection. The signal now carries the value in both directions, with one exception worth knowing about: an empty selection. Clearing the button in code — `button.value = None`, or replacing `options` with a list the current selection is not on — leaves the signal holding the previous value and tells nothing bound to it, where the old label-space binding reported an empty string. Declare the signal `allow_empty=True` and the clear reaches it — see the `Signal` entry under `Added` ([#390](https://github.com/israel-dryer/bootstack/issues/390)). **Check this when you upgrade if you seed a `SelectButton` signal with anything other than one of its option values** — an option's label, or any other string: that now raises, exactly as `value=` already did for the same string, and the fix is to seed it with the option's value instead. This reaches buttons built from plain strings too, where the label and the value are the same text: seeding one with a string that is not on its list used to display that string and now raises. A button whose seed already names an option is unaffected, as is any button with no `signal=`. Reading `button.signal` back now returns the `Signal` you passed in rather than a separate one tracking it. ([#461](https://github.com/israel-dryer/bootstack/issues/461))
...
- **A `Select` bound to a `Signal` now tracks the selection, not the text on screen.** Seeding the signal with an option's value displayed that raw value instead of the option's label, and setting it later moved the displayed text without moving the selection — so the field showed one option while `value` and `selection` still reported the previous one, and no change event fired. That second half applied to plain string options too, and it did not correct itself. The signal now carries the option's value in both directions, matching `value=` and the `signal=` on `NumberField`, `DateField` and `TimeField` — including their one shared exception: clearing the field leaves the signal holding the previous value and tells nothing bound to it, so a cleared `Select` reports `None` to an `on_change` handler while its signal still reads the old selection. The old text-space binding reported an empty string here. Declare the signal `allow_empty=True` and the clear reaches it — see the `Signal` entry under `Added` ([#390](https://github.com/israel-dryer/bootstack/issues/390)). Two things to check when you upgrade — what a `Select` writes into its signal changes from the option's label to its value, and passing `textsignal=` to a `Select` now raises instead of being silently discarded. `SelectButton` had the same defect and is fixed with it — see its entry under `Changed`. ([#458](https://github.com/israel-dryer/bootstack/issues/458))
