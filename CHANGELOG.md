...
- **A `Signal` can now hold an empty value, so clearing a field is no longer silent.** A signal takes its type from the value you create it with, and it had no way to say "nothing" — so a field bound to one kept its last value when it was cleared, and a handler watching that signal never heard about the clear. Pass `allow_empty=True` when the value can also be empty: `bs.Signal(date.today(), allow_empty=True)`. Call `clear()` to empty it — the same verb the fields themselves use — and clearing a bound field now reaches the signal, in both directions. A signal that allows an empty value can also start empty — `bs.Signal(None, allow_empty=True)` — in which case the first real value decides its type, and it stays that type from then on. Empty is `None`, except where the signal *is* the widget's own variable, as it is for a text field or a radio group: a variable holds only strings, so there empty is `''`. Either way the signal agrees with the widget it is bound to, so comparing it against the widget's `value` is safe. Signals you already have are unchanged: without `allow_empty=True` a clear is still skipped and `set(None)` still raises, now with a message pointing at the option. Binding a signal that allows an empty value to a checkbox, switch, toggle button, slider or progress bar raises rather than storing something that is not empty at all — those types have no empty value: a checkbox is off or on, and a slider always has a position. ([#390](https://github.com/israel-dryer/bootstack/issues/390))
