...
- **A double-click also delivers two `on_row_click` events, before the `on_row_double_click`.** This has always been true, but until now `on_row_double_click` never fired on a read-only table, so there was no reason to register both handlers on one. Now that pairing is the natural thing to write — single click selects, double click opens — and the click handler will run twice for every double-click. If that matters for your handler, keep the work in `on_row_click` idempotent, or move it to the double-click handler. (#417)
...
- **Double-clicking a group header in a grouped `DataTable` no longer fires `on_row_double_click` with an empty record.** A group header is not a row and carries no record, but the handler only checked that some tree item was under the pointer, so it emitted a `RowEvent` whose `record` was `{}` and whose `id` was `None` — enough to raise `KeyError` inside a handler doing the documented `e.record["name"]`. With `allow_edit=True` the same double-click also opened a spurious **New Record** dialog. `on_row_click` has always ignored group headers; double-click now matches it. (#420)
- **Right-clicking a group header in a grouped `DataTable` no longer fires `on_row_right_click` with an empty record.** The same defect as above, one handler over, and this one needed no unusual setup — right-click menus are on by default, so any grouped table was affected. A group header also stopped being recorded as the row menu's target, which could leave a later menu command pointed at a row that carries no record. (#418)
...
- **Clicking a `DataTable` row now leaves the keyboard pointed at that row.** Clicking a group header, or clicking any row on a table showing selection checkboxes, toggled the row but did not give the table keyboard focus — so the arrow keys did not continue from what had just been clicked, and a following Space or arrow key did nothing to the table at all. On a table with selection checkboxes no click anywhere took focus, leaving Tab as the only way to start driving it from the keyboard. (#421)
