...
with bs.App(title="Card Demo", padding=20, gap=16) as app:
    ...
    with bs.HStack(gap=12):
        for accent in ("primary", "secondary", "info", "success", "warning", "danger"):
            with bs.Card(accent=accent, padding=12, gap=4):
                bs.Label(accent.title(), font="heading-sm[bold]", accent=accent)
    ...
    bs.Label("Nested Cards", font="heading-sm[bold]")
    with bs.Card(padding=12, gap=8):
        bs.Label("Outer (card surface)", font="heading-sm[bold]")
        with bs.HStack(gap=8):
            with bs.Card(padding=10, gap=4):
                bs.Label("Inner A", font="heading-sm[bold]")
                bs.Label("Overlay surface", font="caption")
            with bs.Card(padding=10, gap=4):
                bs.Label("Inner B", font="heading-sm[bold]")
                bs.Label("Overlay surface", font="caption")
            with bs.Card(accent="primary", padding=10, gap=4):
                bs.Label("Inner C", font="heading-sm[bold]", accent="primary")
