...
with bs.App(title="CodeEditor Demo", size=(800, 700)) as app:
    with bs.ScrollView(fill="both", expand=True):
        with bs.VStack(padding=20, gap=16, fill="x"):

            # Python syntax highlighting
            bs.Label("Python", font="heading-sm")
            bs.CodeEditor(value=SAMPLE_PY, language="python", height=7, fill="x")

            # SQL syntax highlighting
            bs.Label("SQL", font="heading-sm")
            bs.CodeEditor(value=SAMPLE_SQL, language="sql", height=7, fill="x")
            ...
            bs.CodeEditor(value=SAMPLE_PY, language="python", height=6, fill="x", read_only=True)

            # Undo / redo + dirty tracking
            bs.Label("Undo / Redo + Dirty Tracking", font="heading-sm")
            dirty_sig = bs.Signal("not modified")
            editor = bs.CodeEditor(value=SAMPLE_PY, language="python", height=6, fill="x")
            ...
            ed2 = bs.CodeEditor(value=SAMPLE_PY, language="python", height=7, fill="x")
            with bs.HStack(gap=8):
            ...
            ed3 = bs.CodeEditor(value=SAMPLE_PY, language="python", height=6, fill="x")
