...
|---|---|---|---|
| `master` | `Optional[tkinter.Misc]` | `None` | The parent widget |
| `padding` | `int` | `0` | Padding around the frame container |
| `scroll_direction` | `Literal['horizontal', 'vertical', 'both']` | `'vertical'` | Scroll direction - 'vertical', 'horizontal', or 'both' |
| `scrollbar_visibility` | `Literal['always', 'never', 'hover', 'scroll']` | `'always'` | Scrollbar visibility mode: |
| `autohide_delay` | `int` | `1000` | Time in milliseconds before auto-hiding scrollbars
in 'scroll' mode |
| `scrollbar_style` | `str` | `'default'` | The bootstyle for scrollbars (e.g., 'primary',
'success', 'danger') |
| `**kwargs` | `Any` | — | Additional keyword arguments passed to the Text widget |
...
| `text` | `tkinter.Text` | Get the internal text widget |
...
| `destroy()` | Clean up resources and destroy the widget |
