...
def hero():
    with bs.App(title="CodeEditor", padding=20, minsize=(720, 1)) as app:
        ...
        bs.CodeEditor(value=SAMPLE_PY, language="python", height=12, fill="x")
    app.run()
...
def languages():
    with bs.App(title="CodeEditor — Languages", padding=20, gap=12, minsize=(720, 1)) as app:
        app.tk.maxsize(720, 9999)
        bs.CodeEditor(value=SAMPLE_PY,  language="python", height=12, fill="x", width=1)
        bs.CodeEditor(value=SAMPLE_SQL, language="sql",    height=15, fill="x", width=1)
    app.run()
